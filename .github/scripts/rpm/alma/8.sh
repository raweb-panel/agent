...
yum install --allowerasing -y \
    make gcc gcc-c++ cmake3 openssl-devel pcre2-devel bison readline-devel zlib-devel \
...
RPM_REPO_URL="https://repo.julio.al/$UPLOAD_USER/$BUILD_REPO/${RPM_DIST}/"
echo "Checking if $RPM_PACKAGE_FILE_NAME exists at $RPM_REPO_URL..."
...
git clone --depth=1 --branch $SQL_VERSION_MAJOR https://github.com/MariaDB/server.git; cd server/; git submodule update --init --recursive
...
RPM_ROOT="$RPM_BUILD_DIR/BUILDROOT/${RPM_PACKAGE_NAME}-${RPM_VERSION}-${RPM_DIST}.${RPM_ARCH}"
...
if [ ! -d /raweb/apps/mariadb/data/mysql ] || [ -z "\$(ls -A /raweb/apps/mariadb/data/mysql 2>/dev/null)" ]; then
    ...
    /raweb/apps/mariadb/core/bin/mariadb \\
      --socket=/raweb/apps/mariadb/data/mariadb.sock \\
      -u root \\
      -e "DROP USER 'raweb'@'localhost';"

    /raweb/apps/mariadb/core/bin/mariadb \\
      --socket=/raweb/apps/mariadb/data/mariadb.sock \\
      -u root \\
      -e "CREATE DATABASE raweb CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"

    /raweb/apps/mariadb/core/bin/mariadb \\
      --socket=/raweb/apps/mariadb/data/mariadb.sock \\
      -u root \\
      -e "CREATE USER 'root'@'127.0.0.1' IDENTIFIED BY '\$ROOT_PASSWORD';"

    /raweb/apps/mariadb/core/bin/mariadb \\
      --socket=/raweb/apps/mariadb/data/mariadb.sock \\
      -u root \\
      -e "ALTER USER 'root'@'127.0.0.1' IDENTIFIED BY '\$ROOT_PASSWORD';"
...
# The actual generated RPM filename includes the release (BUILD_CODE)
